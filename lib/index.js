...
const doctorTool = {
  ...
  description: 'Self-check the Volcano Ark plan-API activation for DeepSeek v4 flash in DeepSeek Harness. Reads the current config (agent-default-model reasoningEffort, llm-pi-ai providers.ark route: reasoningEfforts + contextWindow 1M + maxTokens 256K), reports the known activation traps (missing reasoningEfforts makes the model non-reasoning; reasoningEffort=max without it throws UNSUPPORTED_REASONING_EFFORT; unset contextWindow/maxTokens silently shrink the model to 256K/32K), and — unless probe:false — live-tests the endpoint to show the server-resolved model version and whether a thinking block comes back. Use when the user asks to verify the ark setup, whether effort=max is active, or what model version ark resolves.',
  parameters: {
    type: 'object',
    properties: {
  ...
  async execute(args) {
    ...
    const contextDeclared = /^\s*contextWindow\s*:\s*(\d+)\s*$/m.exec(ark);
    const maxTokensDeclared = /^\s*maxTokens\s*:\s*(\d+)\s*$/m.exec(ark);
    const contextWindow = contextDeclared ? Number(contextDeclared[1]) : undefined;
    const maxTokens = maxTokensDeclared ? Number(maxTokensDeclared[1]) : undefined;

    // ---- 2. evaluate the traps ----
    if (!effortsDeclared) {
    ...
    if (contextWindow !== 1048576) {
      issues.push(`contextWindow is ${contextWindow ?? '(unset, pi-ai default 262144)'} — the model's real spec is 1M (1048576); without it DSH treats the model as 256K-context.`);
    }
    if (maxTokens !== 262144) {
      issues.push(`maxTokens is ${maxTokens ?? '(unset, pi-ai default 32768)'} — the model's real output cap is 256K (262144); without it output is capped at 32K.`);
    }
    if (admEffort !== 'max') {
    ...
    return {
      ...
      config: {
        ...
        arkRoute: { baseURL: arkBase, api: arkApi, apiKeyEnv: arkKeyEnv, modelId: arkModelId, reasoningEffortsDeclared: effortsDeclared, contextWindow, maxTokens },
      },
